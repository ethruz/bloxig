...
## 🟢🟢🟢 SESSION 2026-07-09→11 — SUBMITTED. ALL DELIVERABLES DONE (NEWEST, supersedes all below)

**BLOXIG IS SUBMITTED to the AMD Hackathon (Track 3 / Unicorn).** Everything is done.
Deadline July 11. Listed publicly as "Bloxig — AI That Makes Roblox UIs Playable" (team
Kayoru, tagged AMD Developer Cloud). Now it's in the judges' hands.

### Final bug fixes (July 10-11, after the deck was done)
1. **Only 1 of N elements wired** (Quest UI: middle Claim worked, others + X didn't).
   Root cause: Kimi is non-deterministic — given 5 elements it wrote a handler for only
   ONE. FIXED in `Generator.lua` with a DETERMINISTIC GAP-FILL: after Kimi returns, the
   plugin checks every detected element's name against the AI output; for any the AI
   omitted, it appends a guaranteed-correct handler (from the roleHint). If the AI fails
   entirely, full deterministic fallback wires everything. So EVERY element is guaranteed
   wired now — "AI writes it, our system verifies + guarantees completeness." Good pitch line.
2. Earlier same-day: mouse-unlock (UserInputService) so cursor shows in Play mode; and
   overlay `Modal=true` + `ZIndex=10000` so clicks land over baked art (diagnosed via
   red-box + manual-handler command-bar tests). All in `Generator.lua` overlayButton.
3. `aiWire.js` hardened stripFences (unwrap fences, strip prose, inject `local root` if
   missing) — guards against Kimi's non-deterministic output. USE_STUB=false, kimi-k2p6.

### DELIVERABLES — ALL DONE
- ✅ **Public repo** `github.com/ethruz/bloxig` (the FULL product, made public per admin
  ruling — "submit all source code, public; private repos not allowed"). Secret-scanned
  clean (no .env tracked, no real keys/JWT in history; the mongodb+srv line was `...`-
  redacted; cluster hostname scrubbed from CLAUDE.md). Has full README (AMD front & center,
  10 mentions) + proprietary all-rights-reserved LICENSE. Debug JSON files removed.
  (The separate clean `bloxig-ai-demo` repo also exists but the FULL repo is what's submitted.)
- ✅ **Slide deck** `Bloxig_Hackathon_Deck.pptx/.pdf` — 9 slides, real pricing (Free/$12/$49),
  scalability slide, Kimi-as-deliberate-choice, honest "real product not prototype" framing.
- ✅ **Demo video** — recorded + edited in CapCut, ~4 min (under 5-min cap). Figma → import →
  click-that-works (Claim reacts, X closes) → AMD/Kimi/Fireworks → close. Honest framing.
- ✅ **Hosted URL** bloxig.onrender.com — live, kept warm via UptimeRobot (5-min pings; the
  "down" flapping is just Render cold-start, site works fine).
- ✅ **Cover image** `bloxig_cover.png` (1280×720, on-brand, blue logo + tagline + AMD line).
- ✅ Submission form: Track 3/Unicorn, categories Designer Tool Plugin + Design, tech
  AMD Developer Cloud, Docker=N/A, Additional Info explains AMD usage + that Kimi isn't
  in the dropdown. Title/short/long descriptions all written.

### RULES CONFIRMED (from admin + guide)
- Pre-existing products ARE allowed (no "must be built during event" rule) — "real product"
  framing is safe. The AI interaction layer is the new hackathon build.
- Auto-screener reads repo + deck (PDF) + hosted URL for AMD usage (or DISQUALIFIED). Video
  is human-judges only. Can re-submit up to 10×/hour. No Docker for Track 3.
- Team registration (Kayoru) done — required to submit. Raw AMD GPU (notebooks.amd.com) NOT
  needed — we use AMD via Fireworks (managed), which fully satisfies the AMD requirement.

### COMPETITIVE READ (from Track 3 submission list)
Bloxig is the ONLY Roblox/game-UI project in the field. Strongest competition = the
"AMD-deep infra" crowd (Klaus/vLLM-ROCm, Chronos/MI300X, PortForge CUDA→ROCm, StyleForge
fine-tune) who touch raw AMD hardware. Bloxig's edge: real shipping product + unique
underserved domain + visceral "click-that-works" demo + novel structure-detection. Realistic
placement: top 10-15, higher if the demo lands. Not a longshot; a complete, differentiated entry.

### POST-SUBMISSION TODO (housekeeping, not urgent)
- ROTATE keys now that full source is public: Fireworks API key, Mongo password, and the JWT
  that got pasted many times in chat (soltinikosolta886 account, Lifetime, exp 2027). Nothing
  leaked, but rotate as belt-and-suspenders.
- Optional: before/after screenshots in README; .DS_Store cleanup.

### THE COLLABORATION MODEL (established, keep using)
Claude analyzes vs real codebase + researches prior art → pull in Gemini/other AIs for 2nd
architecture opinion → Claude cross-checks their answer vs real constraints they can't see →
merge → build → VERIFY (syntax-check, curl, read actual JSON, don't guess) → update CLAUDE.md.
Ship working before polishing. This loop solved the whole structure-detection rebuild.

### SIDE THREADS (parked, user curious — for a FUTURE session, NOT during hackathon)
- **Gray Swan** (grayswan.ai) — legit LLM red-teaming bounty (UK AISI/OpenAI/Anthropic/etc
  backed). Wave-based. Winning technique = subtlety/less-is-more, not brute force. Top-40 →
  invited to private PAID red-teaming network (the real value). Claude teaches methodology/
  strategy, won't write working jailbreak payloads.
- **Bug bounty from methodology PDFs** (Hacker101 etc.) — YES, Claude can deepen methodologies,
  apply to authorized scope, review findings, write reports, teach chaining/IDOR/auth-logic/
  business-logic. Won't exploit unauthorized targets. Upload PDFs + point at a scope.
- **Phone diagnostic tool** (AI hardware-fault detection via current-waveform signatures, B2B
  to repair shops) — MVP buildable (simulate data → scikit-learn classifier → dashboard);
  full product = multi-year DATA-COLLECTION grind (the real moat/bottleneck, Gemini undersold
  this). Underserved gap exists. Fits user's reverse-engineering/kernel-log edge. Future
  hackathon, not now. Full Gemini thread at uploads/Hackathon-II_idea.txt.
- **Mac 2012 kernel/internal inspection** — user curious about safe, non-harmful inspection of
  their own machine. Never fully answered — pick up next session if user wants.

---

## 🟢🟢🟢 SESSION 2026-07-08 — FEATURE WORKING + DECK DONE, VIDEO NEXT (supersedes all below)
