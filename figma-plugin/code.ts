...
interface BloxigNode {
  ...
  // ── Structure-based interactivity (v1.5) — set by detectInteractivity ──
  // interactive: this node should be wired as clickable (detected by STRUCTURE,
  // not name). roleHint: inferred behavior. context: signals for the AI to reason
  // about (position zone, size, text, whether it's part of a tab row).
  interactive?:  boolean;
  roleHint?:     string;   // 'close' | 'claim' | 'tab' | 'action' | 'input' | 'generic'
  uiContext?:    {
    text?:        string;
    zoneX?:       string;   // 'left' | 'center' | 'right'
    zoneY?:       string;   // 'top' | 'middle' | 'bottom'
    aspect?:      number;
    rowMember?:   boolean;  // part of an evenly-spaced sibling row (likely tab)
  };
  children:      BloxigNode[];
}
...
async function handleExport(token: string) {
  ...
  {
    const rabb = (node as any).absoluteBoundingBox;
    __exportRootGeo = rabb
      ? { x: rabb.x, y: rabb.y, w: rabb.width, h: rabb.height }
      : { x: 0, y: 0, w: (node as any).width || 1, h: (node as any).height || 1 };
  }
...
let __exportRootGeo: { x: number; y: number; w: number; h: number } = { x: 0, y: 0, w: 1, h: 1 };
...
function detectInteractivity(
  node: any, base: BloxigNode, parsed: ParsedName, parent: any,
...
function serialiseNode(node: SceneNode, parentAbsX: number, parentAbsY: number): BloxigNode | null {
  ...
  if (shouldRasterizeGroup(node, parsed, false)) {
    ...
    detectInteractivity(
      node, base, parsed, (node as any).parent,
      absX, absY,
      __exportRootGeo.x, __exportRootGeo.y, __exportRootGeo.w, __exportRootGeo.h
    );
